...
if (sim_call_type==sim_childscriptcall_initialization) then
   ...
   simSetObjectName(handleTorso,"TorsoResp")
...
if (sim_call_type==sim_childscriptcall_actuation) then
   ...
   if ticks == 100 then
      ...
      for i=1,#tJointHandle do
	 h=tJointHandle[i]
	 name = simGetObjectName(h)
	 posit = round(simGetJointPosition(h)*radDeg,1)
	 force = simGetJointForce(h)
	 print (i,posit,force,name)
      end
   end

   angPitch = angPitch+angStep
   if angPitch > 140.0 then
      angStep = -1.0
   end
   if angPitch < -140.0 then
      angStep = 1.0
   ...
   for i=1,#tJointHandle do
      ...
      name = simGetObjectName(h)
      ang = 0.0
      if name == "LElbowRoll" then
	 ang = -2.0
      end
      if name == "RElbowRoll" then
	 ang = 2.0
      end
      if name == "LShoulderPitch" then
 	 ang = angPitch
      end
      if name == "RShoulderPitch" then
 	 ang = angPitch
      end
      if name == "HeadYaw" then
 	 ang = 0.0
      end
      if name == "HeadPitch" then
 	 ang = 0.0
      end

      ang = ang*degRad
      simSetJointTargetPosition(h,ang)
      --simSetJointPosition(h,ang)
      simSetJointForce(h,1000.0)    
   end
end
...
if (sim_call_type==sim_childscriptcall_cleanup) then
   ...
   simAddStatusbarMessage("Simulation is been freezed ...")
   modelFileName="/home/newubu/MyApps/Nao/v-rep/nao-new-model/build/vnao/V-REP_PRO_EDU_V3_3_2_64_Linux/models/robots/mobile/ROBOT_MADE_IN_LUA_1.ttm"
   simAddStatusbarMessage("save model to "..modelFileName)
   simSaveModel(handleTorso,modelFileName)
   --sceneFileName="/home/newubu/MyApps/Nao/v-rep/nao-new-model/ROBOT_MADE_IN_LUA_1.ttt"
   --simAddStatusbarMessage("save scene to "..sceneFileName)
   --simSaveScene(sceneFileName)
